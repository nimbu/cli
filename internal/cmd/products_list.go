...
func (c *ProductsListCmd) Run(ctx context.Context, flags *RootFlags) error {
	...
	if err := requireScopes(ctx, client, []string{"read_products"}, "Example: nimbu auth scopes"); err != nil {
		return err
	}
	...
	if mode.Plain {
		return output.PlainFromSlice(ctx, products, listOutputFields(&c.QueryFlags, plainFields))
	}
