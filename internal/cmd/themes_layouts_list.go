...
func (c *ThemeLayoutsListCmd) Run(ctx context.Context, flags *RootFlags) error {
	...
	path := "/themes/" + url.PathEscape(c.Theme) + "/layouts"
	opts, err := listRequestOptions(&c.QueryFlags)
	if err != nil {
		return fmt.Errorf("list theme layouts: %w", err)
	}

	var layouts []api.ThemeResource

	if c.All {
		layouts, err = api.List[api.ThemeResource](ctx, client, path, opts...)
		if err != nil {
			return fmt.Errorf("list theme layouts: %w", err)
		}
	} else {
	...
	if mode.JSON {
		return output.JSON(ctx, layouts)
	}
	...
	if mode.Plain {
		return output.PlainFromSlice(ctx, layouts, listOutputFields(&c.QueryFlags, plainFields))
	}

	fields, headers := listOutputColumns(&c.QueryFlags, tableFields, tableHeaders)
	return output.WriteTable(ctx, layouts, fields, headers)
}
