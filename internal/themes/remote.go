...
func FetchResource(ctx context.Context, client *api.Client, theme string, kind Kind, remoteName string) (api.ThemeResource, error) {
	...
	path := fmt.Sprintf("/themes/%s/%s/%s", url.PathEscape(theme), kind.Collection(), escapeThemeRemoteName(remoteName))
...
func Delete(ctx context.Context, client *api.Client, theme string, resource Resource) error {
	requestPath := fmt.Sprintf("/themes/%s/%s/%s", url.PathEscape(theme), resource.Kind.Collection(), escapeThemeRemoteName(resource.RemoteName))
...
func ReadContent(ctx context.Context, client *api.Client, theme string, kind Kind, remoteName string) ([]byte, error) {
	...
	return readRemoteContent(ctx, client, kind, remoteName, resource.Code, resource.PublicURL)
}

func readResourceContent(ctx context.Context, client *api.Client, theme string, resource Resource) ([]byte, error) {
	publicURL := resource.PublicURL
	code := ""
	if resource.Kind != KindAsset {
		fetched, err := FetchResource(ctx, client, theme, resource.Kind, resource.RemoteName)
		if err != nil {
			return nil, err
		}
		code = fetched.Code
		if publicURL == "" {
			publicURL = fetched.PublicURL
		}
	}
	return readRemoteContent(ctx, client, resource.Kind, resource.RemoteName, code, publicURL)
}

func readRemoteContent(ctx context.Context, client *api.Client, kind Kind, remoteName, code, publicURL string) ([]byte, error) {
	if code != "" {
	...
	if kind == KindAsset && strings.TrimSpace(publicURL) != "" {
		...
		return io.ReadAll(task.WrapReadCloser(resp.Body))
	}
...
func escapeThemeRemoteName(name string) string {
	parts := strings.Split(strings.TrimPrefix(normalizePath(name), "/"), "/")
