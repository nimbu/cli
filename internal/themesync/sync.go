...
func run(ctx context.Context, client *api.Client, cfg Config, opts Options, mode string) (Result, error) {
	...
	uploads, deletes, err := planOperations(ctx, client, cfg, opts, mode, allLocal, localByKey)
	if err != nil {
		return result, err
	}
...
func planOperations(ctx context.Context, client *api.Client, cfg Config, opts Options, mode string, allLocal []Resource, localByKey map[resourceKey]Resource) ([]Resource, []Resource, error) {
